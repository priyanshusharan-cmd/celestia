...
class OrbitalScene(Scene):
    ...
    def construct(self):
        ...
        np.random.seed(42)
        stars = VGroup(*[
            Dot(np.array([np.random.uniform(-6, 6), np.random.uniform(-4, 4), 0]), 
                radius=np.random.uniform(0.01, 0.03), 
                color=WHITE, fill_opacity=np.random.uniform(0.1, 0.8))
            for _ in range(250)
        ])
        self.add(stars)
        ...
        p1 = Circle(radius=r1, color="#ffd166", fill_opacity=1.0)
        p1_label = Text(self.body1_name, font_size=16)
        
        p2 = Circle(radius=r2, color="#5eead4", fill_opacity=1.0)
        p2_label = Text(self.body2_name, font_size=16)
        
        probe = Dot(color="#ecfbff", radius=0.06)
        
        # Trace path inside the rotating frame
        rotating_path = VMobject()
        rotating_path.set_stroke(color="#98e8ff", width=1.5, opacity=0.4)
        
        self.add(rotating_path, p1, p2, probe, p1_label, p2_label)
        ...
        def update_objects(m):
            ...
            probe.move_to(rotated_points[idx])
            
        p1.add_updater(update_objects)
        
        # Add inertial trace behind the probe
        inertial_trace = TracedPath(probe.get_center, stroke_width=2.5, stroke_color="#72e6de", stroke_opacity=0.9)
        self.add(inertial_trace)
        
        self.play(tracker.animate.set_value(1), run_time=6.0, rate_func=linear)
        self.wait(1)
...
def render_trajectory(mu, trajectory, output_file, body1_name, body2_name, m1_mass, m2_mass):
    ...
    scene.render()
    return f"{config.media_dir}/videos/800p60/{output_file}.mp4"
